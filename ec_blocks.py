...
class ECBlock:
    ...
    def __init__(self, circuit, start_index):
        ...
        self.last_measurement_indices = np.array([0] * len(self.stabilizers), dtype=int)
    ...
    def measure_stabilizers(self):
        ...
        for idx in range(len(stabs)):
            mpp_targets = stabilizer_to_mpp_targets(stabs[idx], self.qubits)
            ...
            if self.last_measurement_indices[idx] is not None:
                previous_index = self.last_measurement_indices[idx]
            ...
            self.circuit.append("DETECTOR", detector_targets, [0, idx])
            
            # Update the last measurement index for this stabilizer
            self.last_measurement_indices[idx] = current_index

        self.circuit.append("SHIFT_COORDS", [],[1, 0])
